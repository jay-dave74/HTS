		...
		<div class="login-container">
			...
			<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();" validateOnBlur="<%= false %>">
				...
				<aui:fieldset>
					...
					<aui:input cssClass="clearable" id="loginEmail" label="<%= loginLabel %>" name="login" required="<%= true %>" showRequiredLabel="<%= false %>" type="text" value="<%= login %>">
						<c:if test="<%= authType.equals(CompanyConstants.AUTH_TYPE_EA) %>">
							<aui:validator name="email" />
						</c:if>
					</aui:input>

				<!-- <aui:input name="password" required="<%= false %>" showRequiredLabel="<%= false %>" type="password" value="<%= password %>" />
				 -->	
                  <!-- OTP INPUT BOX -->
					<aui:input name="otp" label="OTP" showRequiredLabel="<%= false %>" type="text" value="">
				...
				<aui:button-row>
					...
					<aui:button type="button" value="Get OTP" id="sendOTPButton" />
				</aui:button-row>
			</aui:form>
...
<portlet:resourceURL id="/sendOtp" var="otpResourceURL" />
<script>
	jQuery('#<portlet:namespace/>sendOTPButton').click(function() {
		var email = $('#<portlet:namespace/>loginEmail').val();
		console.log(email)
		console.log("click"+email)

		jQuery.ajax({
			url : '${otpResourceURL}' + '&<portlet:namespace/>email=' + email,
			type : 'get',
